// components/PMaintenance/EditWorkOrderForm.tsx
...
export const EditWorkOrderForm: React.FC<EditWorkOrderFormProps> = ({
  isOpen,
  onOpenChange,
  workOrder,
  onSubmit,
  ...
  // Load work order data when modal opens
  useEffect(() => {
    if (workOrder && isOpen) {
      setTitle(workOrder.title || "");
      setDescription(workOrder.description || "");
      
      // Handle plant - if superadmin, show selected, if admin, show their plant
      if (isSuperAdmin) {
        const plantId = typeof workOrder.plant === "string" ? workOrder.plant : workOrder.plant?._id;
        setSelectedPlant(plantId || "");
      }
      
      setScheduledDate(
        workOrder.scheduledDate 
          ? new Date(workOrder.scheduledDate).toISOString().slice(0, 10)
          : ""
      );
      setPriority(workOrder.priority || "medium");
      setRecurrence(workOrder.recurrence || "none");
      setCycles(workOrder.cycles || 1);
      setNotes(workOrder.notes || "");
      
      // Load tasks
      if (workOrder.tasks && workOrder.tasks.length > 0) {
        const formattedTasks = workOrder.tasks.map((task: any) => ({
          id: task._id || generateUUID(),
          taskName: task.taskName || "",
          description: task.description || "",
          estimatedDuration: task.estimatedDuration || "",
          procedures: task.procedures?.map((p: any, index: number) => ({
            stepNumber: p.stepNumber || index + 1,
  ...
  const handleSubmit = async () => {
    ...
    await onSubmit(workOrder._id, payload);
    onOpenChange(false);
  };

  // Check if work order is completed (should not be editable if completed)
    ...
    <Dialog.Root open={isOpen} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 w-full max-w-3xl -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl p-6 z-50 max-h-[90vh] overflow-y-auto shadow-xl">
          ...
          <h2 className="text-2xl font-bold mb-1">Edit Work Order</h2>
          <p className="text-gray-500 text-sm mb-6">
            Update work order details
            {isCompleted && (
              <span className="ml-2 text-red-500 font-medium">
                (Work Order is {workOrder.status} - changes may be limited)
              </span>
            )}
          </p>

          <div className="space-y-5">
            {/* Basic Info - Same as Create Form but pre-populated */}
            ...
            {/* Tasks Section - Same as Create Form */}
            <div>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              >
                {saving ? "Saving..." : "Update Work Order"}
              </button>
            </div>
          </div>
        </Dialog.Content>
