...
import { ADMIN_DASHBOARD_ROUTE, DASHBOARD_ROUTE, SUPERVISOR_DEVICES_ROUTE } from "../../router/routeConstants";
  ...
  const filteredDevices = (devices ?? []).filter((d) => {
    ...
    return (
      ...
      (selectedDepartment
        ...
        : true) &&
      (selectedPlant ? d.plant?._id === selectedPlant : true)
    );
  });
          ...
          filteredDevices.map((device) => (
            <DeviceCard2
              ...
              onEdit={(id) => 
                {
            if (currentUser?.role === "supervisor") {
              navigate(SUPERVISOR_DEVICES_ROUTE +`/devices/editDevice/${device._id}`);
            } else if (currentUser?.role === "admin") {
              navigate(ADMIN_DASHBOARD_ROUTE + `/devices/editDevice/${device._id}`);
            } else if (currentUser?.role === "superadmin") {
              navigate(DASHBOARD_ROUTE + `/devices/editDevice/${device._id}`);
            }
          }
